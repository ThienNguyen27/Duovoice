...
export default function Practice() {
  ...
  useEffect(() => {
    ...
    if (detector && graphModel && webcamRef.current && videoLoaded && canvasRef.current) {
      const run = async () => {
        ...
        // Clear canvas and assume no hand
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (!isNaN(idx) && idx >= 1 && idx <= suggestions.length) applySuggestion(suggestions[idx - 1]);
    };
    window.addEventListener('keydown', handleKeyDown);
    return () => window.removeEventListener('keydown', handleKeyDown);
  }, [suggestions]);

  // Append predicted letter or control
  ...
  const drawFeedback = (ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement, score: number) => {
    ctx.lineWidth = 5;
  ...
  return (
    <div className='flex flex-col items-center p-4 space-y-4'>
      ...
      <div className='w-full max-w-lg flex items-center justify-between'>
        ...
        <button onClick={fetchPhrase} className='px-3 py-1 bg-green-600 text-white rounded hover:bg-green-700'>Next Phrase</button>
      </div>
      <p className='w-full max-w-lg p-3 border rounded bg-gray-50 whitespace-pre-wrap'>{phrase || 'Loading...'}</p>
      ...
      <div className='flex items-center space-x-2'>
        <button onClick={appendPredicted} className='px-4 py-2 bg-blue-600 text-white rounded'>Write Letter</button>
        <button onClick={() => setInputText(t => t + ' ')} className='px-4 py-2 bg-indigo-600 text-white rounded'>Space</button>
        <button onClick={() => setInputText(t => t.slice(0, -1))} className='px-4 py-2 bg-red-600 text-white rounded'>Delete</button>
      </div>
      ...
      {suggestions.length > 0 && (
        <div className='w-full max-w-lg'>
          ...
          <ul className='grid grid-cols-2 gap-2'>
            {suggestions.map((s, i) => (
              <li key={i} className='p-2 border rounded cursor-pointer hover:bg-gray-100' onClick={() => applySuggestion(s)}>
      ...
      <div>
        {videoError ? <span className='text-red-600'>Error: {videoError}</span>
        : videoLoaded ? <span className='text-green-600'>Webcam active</span>
        : <span className='text-gray-500'>Waiting for webcam...</span>}
      </div>

      {/* Prediction & score */}
      <div className='text-2xl font-semibold'>Prediction: <span className='text-blue-600'>{prediction}</span></div>
      <div className='text-lg'>Score: {matchScore}%</div>

      {/* Gamification panel */}
      <div className='ml-6 flex flex-col space-y-4'>
        <div><h2 className='text-2xl font-semibold'>Scoreboard</h2><p>Points: <strong>{points}</strong></p><p>Streak: <strong>{streak}</strong></p></div>
        <div><h2 className='text-2xl font-semibold'>XP & Level</h2><div className='w-64 bg-gray-300 rounded-full h-4 overflow-hidden'><div className='h-full rounded-full' style={{ width: `${(xp / (level * 1000)) * 100}%` }} /></div><p className='mt-1'>Level {level} ({xp}/{level * 1000} XP)</p></div>
      </div>
    </div>
  );
}